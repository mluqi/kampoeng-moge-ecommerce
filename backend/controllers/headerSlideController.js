...
exports.createHeaderSlide = async (req, res) => {
    try {
        const { title, offer_text, button1_text, button1_link, button2_text, button2_link, display_order, is_active } = req.body;
        ...
        const newSlide = await HeaderSlide.create({
            title, offer_text, image_url, button1_text, button1_link, button2_text, button2_link,
            display_order: parseInt(display_order, 10) || 0,
            is_active: is_active === 'true' || is_active === true,
        });

        res.status(201).json({ message: 'Slide berhasil ditambahkan.', slide: newSlide });
    } catch (error) {
...
exports.updateHeaderSlide = async (req, res) => {
    try {
        ...
        if (!slide) return res.status(404).json({ message: 'Slide tidak ditemukan.' });

        const updateData = {
            title, offer_text, button1_text, button1_link, button2_text, button2_link,
            display_order: parseInt(display_order, 10) || 0,
            is_active: is_active === 'true' || is_active === true,
        };
        ...
        if (req.file) {
            if (slide.image_url) {
                ...
                const oldImagePath = path.join(__dirname, '..', slide.image_url.substring(1));
...
exports.deleteHeaderSlide = async (req, res) => {
    try {
        ...
        if (slide.image_url) {
            ...
            const imagePath = path.join(__dirname, '..', slide.image_url.substring(1));
